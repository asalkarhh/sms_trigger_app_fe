...
export default function QuickInfo({ business }) {
  const { address, phone, alternatePhone, whatsapp, mapLink } = business
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3 sm:gap-8">
          ...
          {address && (
            <a href={mapLink} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm font-semibold tracking-wide text-slate-900 line-clamp-2 transition group-hover:text-accent">{address}</p>
              </div>
            </a>
          )}
          ...
          {whatsapp && (
            <div className="group flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-medium text-slate-500">Chat with us</p>
                <a href={whatsappLink(whatsapp)} target="_blank" rel="noopener noreferrer" className="text-base font-semibold tracking-wide text-slate-900 transition hover:text-[#25D366]">WhatsApp</a>
              </div>
            </div>
          )}
